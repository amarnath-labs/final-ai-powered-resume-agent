...
def setup_sidebar():
     with st.sidebar:
        ...
        # CHANGED: Use Groq API Key instead of OpenAI
        ...
        st.markdown(
            ...
            <div style="text-align: center; margin-top: 20px;">
                ...
                <p style="font-size: 0.8rem; color: #666;">v1.0.0 - Powered by Groq AI</p>
            </div>
            """,
            unsafe_allow_html=True
        )
     ...
     return {
        "groq_api_key": groq_api_key,  # CHANGED from openai_api_key
...
def role_selection_section(role_requirements):
    ...
    with col2:
        upload_jd = st.checkbox(
            "Upload custom job description instead"
        )
    ...
    if not upload_jd:
        st.info(
            f"Required skills: {', '.join(role_requirements[role])}"
        )

        st.markdown(
            f"""
            <p>
                Cutoff Score for selection:
                <b>75/100</b>
            </p>
            """,
            unsafe_allow_html=True
...
def setup_custom_css():
    """
    Injects the necessary CSS to style the custom HTML components 
    (cards, tags, buttons) used in the analysis results.
    """
    st.markdown("""
    <style>
        .card {
        ...
        .skill-tag {
            display: inline-block;
            background-color: #2E7D32; /* Green for strengths */
            color: white;
            padding: 5px 10px;
            border-radius: 15px;
            margin: 5px;
        ...
        .skill-tag.missing {
            background-color: #C62828; /* Red for weaknesses */
        }
        .weakness-detail {
...
# --- 2. Main Analysis Display Function ---
        ...
        for weakness in detailed_weaknesses:
            ...
            with st.expander(f"{skill_name} (Score: {score}/10)"):
                ...
                if 'suggestions' in weakness and weakness['suggestions']:
                    st.markdown("<strong>How to improve:</strong>",
                                unsafe_allow_html=True)
                ...
                if 'example' in weakness and weakness['example']:
                    st.markdown("<strong>Example addition:</strong>",
                                unsafe_allow_html=True)
                ...
                for area, suggestions in improvements.items():
                    with st.expander(f"Improvements for {area}", expanded=True):
                        ...
                        if "before_after" in suggestions:
                            ...
                            st.markdown('<div class="comparison-box">', 
                                        unsafe_allow_html=True)
                            st.markdown("<strong>Before:</strong>", 
                                        unsafe_allow_html=True)
                            ...
                            st.markdown('<div class="comparison-box">', 
                                        unsafe_allow_html=True)
                            st.markdown("<strong>After:</strong>", 
                                        unsafe_allow_html=True)
...
def improved_resume_section(has_resume, get_improved_resume_func=None):
    ...
    highlight_skills = st.text_area("Paste your JD to get updated Resume",
    ...
    if st.button("Generate Improved Resume"):
        if get_improved_resume_func:
            with st.spinner("Creating improved resume..."):
                improved_resume = get_improved_resume_func(target_role,
                highlight_skills)
