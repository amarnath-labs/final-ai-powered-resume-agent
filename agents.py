...
class ResumeAnalysisAgent:
    def __init__(self, cutoff_score=75):
        ...
        # CHANGED: Use free HuggingFace embeddings instead of OpenAI
    ...
    def create_rag_vector_store(self, text):
        ...
        # CHANGED: Use HuggingFace embeddings
    ...
    def analyze_resume_weaknesses(self):
        """Analyze resume weaknesses in the resume based on missing skills"""
    ...
    def semantic_skill_analysis(self, resume_text, skills):
        """Analyze skills semantically using RAG + LLM scoring"""

        vectorstore = self.create_vector_store(resume_text)
        retriever = vectorstore.as_retriever()

        # CHANGED: Use ChatGroq
        llm = ChatGroq(model="llama-3.1-70b-versatile", api_key=self.api_key)
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = list(
                executor.map(
                    lambda skill: self.analyze_skill(qa_chain, skill),
                    skills
                )
            )
        ...
        reasoning = (
            "Candidate evaluated based on explicit resume content using "
            "semantic similarity and clear numeric scoring."
        )
    ...
    def analyze_resume(self, resume_file, role_requirements=None, custom_jd=None):
        """Analyze a resume against role requirements or a custom job description"""
    ...
    def generate_interview_questions(self, question_types, difficulty, num_questions):
        ...
        try:
            ...
            interview questions for this candidate based on their resume and skills.
    ...
    def improve_resume(self, improvement_areas, target_role=""):
        ...
        try:
            ...
            remaining_areas = [
                area for area in improvement_areas
            ...
            if remaining_areas:
                ...
                prompt = f"""
                Provide detailed suggestions to improve this resume in the following areas:
                ...
                3. Where relevant, a before/after example
...
Format the resume in a modern, clean style with clear section headings."""
    ...
    def cleanup(self):
        """Clean up temporary files created during resume analysis."""
        try:
